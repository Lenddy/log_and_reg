...
import re
from flask_app.config.connect_to_mysql import connectToMySQL
...
#adds on user when they create a account
    ...
    def add_one(cls,data):
        query = "insert into registration (f_name,l_name,email,password) values(%(f_name)s,%(l_name)s,%(email)s,%(password)s);"
...
# for validating  info that the user inputs
    ...
    def validate(user):
        ...
        if len(user["email"]) == 0:
            ...
            is_valid = False
        elif not registration.match(user["email"]): 
